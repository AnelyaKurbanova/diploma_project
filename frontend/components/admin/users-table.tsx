    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-3">
        <input
          ...
          className="rounded-lg border border-gray-200 px-3 py-2 text-sm outline-none focus:border-brand-navy/50 focus:ring-1 focus:ring-brand-navy/25"
        />
        <select
          value={roleFilter}
            ...
            {loading ? (
              ...
              users.map((u) => (
                <tr key={u.id} className="border-b border-gray-50 hover:bg-gray-50/40">
                  <td className="px-4 py-3 font-medium text-brand-navy">{u.email}</td>
                  <td className="px-4 py-3">
                    <select
                      value={u.role}
                      disabled={changingRole === u.id}
                      onChange={(e) => handleRoleChange(u.id, e.target.value)}
                      className="rounded-md border border-gray-200 px-2 py-1 text-xs outline-none focus:border-brand-navy/50 disabled:opacity-50"
                    >
                      {ROLE_OPTIONS.map((r) => (
                        <option key={r.value} value={r.value}>{r.label}</option>
                      ))}
